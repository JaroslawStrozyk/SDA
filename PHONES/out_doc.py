...
def phonepp(request, pk):
    sprzet = Telefon.objects.get(id=pk)
    ...
    p.setFont("Dejavu", 10)
    txt = 'Sporządzony dnia '+str(sprzet.data.strftime('%d.%m.%Y'))+' r.'
    ...
    opis1 = 'Telefon typ: '+sprzet.model+', IMEI: '+sprzet.imei +', '
    opis2 = 'Nr MSISDN: '+sprzet.msisdn
    opis = opis1 + opis2
    if len(opis) > 79:
        opis = opis1 +'\n'+ opis2
    data=[('1', opis ,'1'),]
...
def phonepz(request, pk):
    ...
    p.setFont("Dejavu", 10)
    txt = 'Sporządzony dnia '+str(sprzet.pz.strftime('%d.%m.%Y'))+' r.'
    p.drawString(((width/2) - 80), (height - 200), txt)
