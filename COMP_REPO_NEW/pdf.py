...
import os
from .models import Sklad, ElementKatalogowy, HistoriaPalety
...
def sklad_pdf_out(request, pk, mag):
    ...
    for s in skl:
        ...
        zdjecie1 = s.element_katalogowy.przech_zdjecie
        zdjecie2 = s.element_katalogowy.przech_zdjecie2
...
def nkat_pdf_naglowek(p, firma, nazwa, opis, paleta, szer, gl, pole):
    im = Image.open(os.path.join(settings.STATIC_ROOT, 'img/sde.png'))
    p.drawInlineImage(im, 30, 738, width=100, height=85)
    ...
    f = Table(data, rowHeights=20, colWidths=[70, 380])
    f.setStyle(TableStyle([
        ('GRID', (0, 0), (-1, -1), 0.15, colors.gray),
        ('FONT', (0, 0), (-1, -1), 'Dejavu', 10),
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
    ...
    f = Table(data, rowHeights=20, colWidths=[ 70, 60])
    f.setStyle(TableStyle([
        ('GRID', (0, 0), (-1, -1), 0.15, colors.gray),
        ('FONT', (0, 0), (-1, -1), 'Dejavu', 10),
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
    ...
    data = [
        ('Zajęta strefa', 'Szerokość:', szer+' m', 'Głębokość:', gl+' m', 'Powierzchnia:', pole+' m²')
    ]
    f = Table(data, rowHeights=20, colWidths=[105, 85, 60, 85, 60, 85, 60])
    f.setStyle(TableStyle([
        ('GRID', (0, 0), (-1, -1), 0.15, colors.gray),
        ('FONT', (0, 0), (-1, -1), 'Dejavu', 10),
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
...
def nkat_pdf_out(request, pk, mag):

    ek = ElementKatalogowy.objects.get(pk=pk)
    title = ek.nazwa
    firma = ek.firma.nazwa
    nazwa = ek.nazwa
    opis  = ek.opis

    # Odczyt najnowszego numeru palety z HistoriaPalety
    ostatnia_paleta = HistoriaPalety.objects.filter(
        sklad__element_katalogowy=ek
    ).order_by('-data_zmiany').first()

    paleta = ostatnia_paleta.numer_palety if ostatnia_paleta else ""
    szer = str(ek.szerokosc)
    gl =   str(ek.glebokosc)
    pole = str(ek.powierzchnia)
    zdjecie1 = ek.przech_zdjecie
    zdjecie2 = ek.przech_zdjecie2

    # dokument

    response = HttpResponse(content_type='application/pdf')
    titlefile = "Element_" + title + '.pdf'
    namefile = 'filename="' + titlefile + '"'
    response['Content-Disposition'] = namefile
    ...
    nkat_pdf_naglowek(p, firma, nazwa, opis, paleta, szer, gl, pole)
    pdf_zdjecia(p, zdjecie1, zdjecie2)
